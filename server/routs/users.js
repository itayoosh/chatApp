...
const {getContactName}=require("../functions/getContactName");
...
router.get("/searchUsers", auth, async (req, res) => {
  try {
    ...
    const data = await userModel
      .find(sFind, { password: 0 })
      .limit(limit)
      .skip(page * limit);

    const dataWithContactNames = await Promise.all(
      data.map(async (contact) => {
        const contactName = await getContactName(userPhone, contact.phone);
        return { ...contact.toObject(), contactName };
      })
    );

    res.json(dataWithContactNames);
  } catch (err) {
